# ============================================
# AI Infrastructure Ranking v7.47.2-US
# FIX: Levermann.txt wird aus Repo gelesen falls vorhanden
# ============================================
...
st.set_page_config(page_title="AI Infrastructure Ranking v7.47.2-US", layout="wide")
VERSION = "v7.47.2-US"
...
# ============================================
# 2. STOCK_UNIVERSE v7.47.2-US - 37 WERTE
# ============================================
STOCK_UNIVERSE = [
...
WEIGHTS = {
    "Empfänger": {'Forward_KGV': 0.10, 'EV_EBITDA': 0.05, 'Umsatz_Wachstum': 0.30, 'Bruttomarge': 0.10, 'Operating_Margin': 0.30, 'FCF_Marge': 0.05, 'NetDebt_EBITDA': 0.05, 'Performance_52W': 0.05},
    "Spender": {'Forward_KGV': 0.15, 'EV_EBITDA': 0.10, 'Umsatz_Wachstum': 0.05, 'Bruttomarge': 0.15, 'Operating_Margin': 0.25, 'FCF_Marge': 0.20, 'NetDebt_EBITDA': 0.05, 'Performance_52W': 0.05},
    "Neutral": {'Forward_KGV': 0.15, 'EV_EBITDA': 0.15, 'Umsatz_Wachstum': 0.15, 'Bruttomarge': 0.15, 'Operating_Margin': 0.15, 'FCF_Marge': 0.15, 'NetDebt_EBITDA': 0.05, 'Performance_52W': 0.05}
}

PFLICHT_KPIS = ["Forward_KGV","EV_EBITDA","Umsatz_Wachstum","Bruttomarge","Operating_Margin","FCF_Marge","Performance_52W","NetDebt_EBITDA","Levermann"]
KPI_LABELS = {"Forward_KGV": "Forward KGV","EV_EBITDA": "EV/EBITDA","Umsatz_Wachstum": "Umsatzwachstum","Bruttomarge": "Bruttomarge","Operating_Margin": "Operating Margin","FCF_Marge": "FCF Marge","Performance_52W": "Performance 52 Wochen","NetDebt_EBITDA": "Net Debt/EBITDA","Levermann": "Levermann Score","Aktueller_Kurs": "Aktueller Kurs"}

LEVERMANN_FILE = "Levermann.txt"

def lade_levermann_aus_datei():
    """Prüft ob Levermann.txt existiert und lädt sie"""
    levermann_dict = {}
    try:
        # FIX: Direkt öffnen, ohne os.path.exists. Klappt auch in Streamlit Cloud
        ...
        st.success(f"Levermann.txt geladen: {len(levermann_dict)} Werte")
    except FileNotFoundError:
        st.warning("Levermann.txt nicht gefunden. Bitte ins Repo hochladen.")
    except Exception as e:
        st.error(f"Fehler beim Laden von Levermann.txt: {e}")
...
def speichere_levermann_in_datei():
    """Speichert alle Levermann Scores in die TXT"""
    lines = []
    ...
    try:
        ...
        st.error(f"Konnte Levermann.txt nicht speichern: {e}")
#Ende Block 1
# ============================================
# 3. HELPER
# ============================================
...
def yahoo_laden(ticker):
    try:
        ...
        currency = safe_get(info, "currency"); forward_kgv = safe_get(info, "forwardPE"); ev_ebitda = safe_get(info, "enterpriseToEbitda")
        umsatz_wachstum = safe_get(info, "revenueGrowth"); brutto = safe_get(info, "grossMargins"); op_marge = safe_get(info, "operatingMargins")
        fcf = safe_get(info, "freeCashflow"); revenue = safe_get(info, "totalRevenue")
        ...
        perf_52w = safe_get(info, "52WeekChange"); total_debt = safe_get(info, "totalDebt"); total_cash = safe_get(info, "totalCash"); ebitda = safe_get(info, "ebitda")
        ...
        return {"Aktueller_Kurs": price, "Waehrung": currency, "Forward_KGV": forward_kgv, "EV_EBITDA": ev_ebitda, "Umsatz_Wachstum": umsatz_wachstum, "Bruttomarge": brutto, "Operating_Margin": op_marge, "FCF_Marge": fcf_marge, "Performance_52W": perf_52w, "NetDebt_EBITDA": netdebt_ebitda}
...
def baue_abfrage_queue():
    ...
    for ticker in st.session_state.aktien_liste:
        ...
        if "Levermann" not in fehlende: fehlende.append("Levermann")
        for kpi in fehlende: queue.append((ticker, kpi))
    st.session_state.abfrage_queue = queue

# ============================================
# 4. SCORING ENGINE v7.47.2
# ============================================
def get_levermann_multiplikator(wert):
    if pd.isna(wert): return 1.0
    if wert > 6: return 1.4
    if wert > 5: return 1.3
    if wert > 4: return 1.2
    if wert > 3: return 1.1
    if wert == 3: return 1.0
    if wert < 3: return 0.8
    return 1.0
...
def calculate_scores(df):
    for kpi in PFLICHT_KPIS:
        if kpi not in df.columns: df[kpi] = np.nan
    ...
    df['Levermann_Mult'] = df['Levermann'].apply(get_levermann_multiplikator)
    ...
    for idx, row in df.iterrows():
        weights = WEIGHTS[row['typ']]; score = 0
    ...
    df['Gesamtscore'] = (df['Gesamtscore_Roh'] * df['Levermann_Mult'] * (0.3 + 0.7 * df['Datenqualität']) + df['Capex_Bias']).round(1)
...
def highlight_na(val): return 'background-color: #FFF9C4' if pd.isna(val) else ''
#Ende Block 2
# ============================================
# 5. SCREENS
# ============================================
def screen_sammeln():
    ...
    if st.button("✅ Auswertung starten", type="primary", use_container_width=True):
        with st.spinner("Lade Yahoo Daten + prüfe Levermann.txt..."):
            lade_levermann_aus_datei() # FIX: Prüft Datei und lädt
            baue_abfrage_queue()
...
def screen_abfrage():
    ...
    with col1:
        if st.button("💾 Speichern/Überschreiben"):
            ...
            if kpi == "Levermann": speichere_levermann_in_datei()
...
def screen_ranking():
    ...
    st.subheader("Globales Ranking v7.47.2-US")
    ...
    show_cols = ['Rang', 'Ticker', 'name', 'flag', 'segment', 'typ', 'Capex_Bias', 'Aktueller_Kurs', 'Forward_KGV', 'EV_EBITDA', 'Umsatz_Wachstum', 'Bruttomarge', 'Operating_Margin', 'FCF_Marge', 'NetDebt_EBITDA', 'Performance_52W', 'Levermann', 'Finanzscore', 'Gesamtscore', 'Investment_Rating']
    for col in show_cols:
        if col not in df_show.columns: df_show[col] = np.nan
    df_show = df_show[show_cols]
    format_dict = {c: (lambda x: "N/A" if pd.isna(x) else f"{x:.2f}") for c in PFLICHT_KPIS + ['Aktueller_Kurs', 'Finanzscore', 'Gesamtscore']}
    format_dict['Rang'] = lambda x: f"{int(x)}"; format_dict['Capex_Bias'] = lambda x: f"{int(x):+d}P"
    styled_df = df_show.style.map(highlight_na).format(format_dict)
    st.table(styled_df)
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer: df.to_excel(writer, index=False, sheet_name="Ranking_v7.47.2")
    st.download_button("📥 Excel herunterladen", output.getvalue(), file_name=f"AI_Ranking_v7.47.2_US_{datetime.now().strftime('%Y-%m-%d')}.xlsx", use_container_width=True)
    if st.button("⬅️ Zurück zur Liste"): st.session_state.modus = "sammeln"; st.rerun()
