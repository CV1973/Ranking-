# ============================================
# AI Infrastructure Return Ranking v15.2
# FIX: Input State Bug, Versions-Reset, Kein Reload nach manueller Eingabe
# ============================================
...
st.set_page_config(page_title="AI Return Ranking v15.2", layout="wide")
VERSION = "v15.2"
...
if "ranking_start" not in st.session_state:
    st.session_state.ranking_start = False # FEHLER 2 FIX

if "web_vorschlaege" not in st.session_state:
    st.session_state.web_vorschlaege = {}

# SICHERHEITSRESET BEI VERSIONSWECHSEL
if st.session_state.get("version_loaded")!= VERSION:
    st.session_state.datenbank = {}
    st.session_state.ticker_index = 0
...
KPI_LABELS = {
    ...
    "Umsatz_Wachstum":"Umsatzwachstum", "OpMarge":"Operative Marge", "FCF_Marge":"FCF Marge", "Performance_52W":"52 Wochen Performance"
}

KPI_HINTS = {
    "Forward_KGV":"z.B. 25.4", "PEG":"z.B. 0.8", "EV_EBITDA":"z.B. 18",
    "FCF_Yield":"z.B. 0.05 = 5%", "Umsatz_Wachstum":"z.B. 0.15 = 15%",
    "OpMarge":"z.B. 0.30 = 30%", "FCF_Marge":"z.B. 0.20 = 20%",
...
def init_ticker(ticker):
    if ticker not in st.session_state.datenbank:
        st.session_state.datenbank[ticker] = {
            "daten":{"Ticker":ticker, "Name":NAMEN.get(ticker,ticker)},
            "audit":{},
            "status":"neu"
        }
...
def yahoo_laden(ticker):
    try:
        ...
        return {
            ...
            "Umsatz_Wachstum":umsatz_wachstum, "OpMarge":op_marge, "FCF_Marge":fcf_marge, "Performance_52W":performance
        }
    except: return None
...
def ticker_laden(ticker):
    ...
    # FEHLER 3 FIX: Nicht neu laden wenn schon geprüft oder fertig
    if obj["status"] in ["pruefen", "fertig", "übersprungen"]:
        return
...
def kpi_assistent(ticker):
    ...
    # FEHLER 1 FIX: value="" damit State sauber ist
    eingabe = st.text_input(
        "Wert eingeben", key=f"input_{ticker}_{kpi}",
        placeholder="z.B. 0.08", value=""
    )
    ...
    with col1:
        if st.button("💾 Speichern", key=f"save_{ticker}_{kpi}"):
            # FEHLER 1 FIX: Direkt aus session_state holen
            raw = st.session_state.get(f"input_{ticker}_{kpi}", "")
            try:
                wert = parse_number(raw)
                if pd.isna(wert):
                    st.error("Bitte gültige Zahl eingeben")
                    return False
...
def assistenten_lauf():
    ...
    ticker_laden(ticker) # wird jetzt nicht mehr bei "pruefen" neu geladen
    obj = st.session_state.datenbank[ticker]
...
def berechne_ranking(df):
    ...
    for i in range(len(df)):
        if i < len(df)*0.2: ratings.append("STRONG BUY")
        elif i < len(df)*0.5: ratings.append("BUY")
        else: ratings.append("HOLD")
    df["Rating"]=ratings
    return df
...
with pd.ExcelWriter(output, engine="openpyxl") as writer:
    df.to_excel(writer, index=False, sheet_name="AI_Ranking_v15.2")

st.download_button("📥 Excel herunterladen", output.getvalue(), file_name=f"AI_Ranking_v15.2_{datetime.now().strftime('%Y-%m-%d')}.xlsx")
